...
const SIZE: Point = Point { x: 80, y: 15 };
const BLOCK: char = '*';
const SPACE: char = ' ';

type PlayingArea = [[char; SIZE.x]; SIZE.y];
...
fn generate_world<F: Fn((usize, usize)) -> bool>(cell_generation: F) -> PlayingArea {
  let mut world = [[SPACE; SIZE.x]; SIZE.y];
...
fn count_neighbors(world: PlayingArea, x: usize, y: usize) -> u8 {
  let mut count = 0;
  ...
  for i in -1..=1 {
    for j in -1..=1 {
      ...
      if (rel_x >= 0 && rel_x < SIZE.x as isize)
        && (rel_y >= 0 && rel_y < SIZE.y as isize)
...
mod tests {
  ...
  fn number_of_neighbors() {
    let empty_world = generate_world(|_| false);
    let filled_world = generate_world(|_| true);

    assert_eq!(count_neighbors(empty_world, 0, 0), 0);
    assert_eq!(count_neighbors(empty_world, 1, 1), 0);
    assert_eq!(count_neighbors(empty_world, SIZE.x - 1, SIZE.y - 1), 0);

    assert_eq!(count_neighbors(filled_world, 0, 0), 3);
    assert_eq!(count_neighbors(filled_world, SIZE.x - 1, SIZE.y - 1), 3);
